        ...
        <SheetContent side="right" className="w-[340px] sm:w-[400px] overflow-y-auto">
          ...
          <div className="mt-6 space-y-6 px-[8px] px-[16px] py-[0px]">
            ...
            {(user || !user) && (
              <>
                <Separator />
                {/* Debug Badge Section - TEMPORARY */}
                <div className="space-y-4 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
                  <h3 className="text-[14px] font-medium text-yellow-600 dark:text-yellow-400">
                    🔧 Badge Debug (Temporary)
                  </h3>
                  <div className="space-y-2">
                    <p className="text-[12px] text-muted-foreground">
                      API Supported: {'setAppBadge' in navigator ? '✅ Yes' : '❌ No'}
                    </p>
                    <p className="text-[12px] text-muted-foreground">
                        ...
                        onClick={() => {
                          if ('setAppBadge' in navigator) {
                            (navigator as any).setAppBadge(1)
                              .then(() => toast.success('Badge set to 1'))
                        ...
                        onClick={() => {
                          if ('setAppBadge' in navigator) {
                            (navigator as any).setAppBadge(5)
                              .then(() => toast.success('Badge set to 5'))
                              .catch((e: Error) => toast.error(`Failed: ${e.message}`));
                          } else {
                            toast.error('Badging API not supported');
                          }
                        ...
                        onClick={() => {
                          if ('clearAppBadge' in navigator) {
                            (navigator as any).clearAppBadge()
                              .then(() => toast.success('Badge cleared'))
                              .catch((e: Error) => toast.error(`Failed: ${e.message}`));
                          } else {
                            toast.error('Clear badge not supported');
                          }
                        }}
                      >
                        Clear Badge
                    ...
                    <p className="text-[11px] text-muted-foreground italic">
                      After clicking, close this app and check the icon on your Home Screen.
                    </p>
                  </div>
                </div>
